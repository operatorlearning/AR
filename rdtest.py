...
def FD_RD_nonlinear(f, Nd, Nt, Nx, nu1=0.01, nu2=0.1):
    # Initial conditions
    dx = L / Nx
    dt = T / Nt
    ...
    for k in range(Nd):
        ...
        for n in range(Nt):
            ...
            for j in range(1, Nx):
                reac = nu2 * un[j] ** 2
                diff = nu1 * (un[j + 1] - 2 * un[j] + un[j - 1]) / (dx ** 2)
                u_new[j] = un[j] + dt * reac + dt * diff + dt * f[k, n, j]
            # Update boundary conditions
            u_new[0] = u_new[-1] = 0  # For example, fixed boundary conditions
            u[k, n + 1] = u_new
    return u


def vec_to_grid(x, N):  # Returns array with size (N,N)
    res = np.zeros((N, N))
    if N ** 2 == x.shape[0]:
        for i in range(N):
            res[i] = x[i * N:(i + 1) * N].T
...
def generate(samples=10, begin=0, end=1, random_dim=11, Nt=100, Nx=20, length_scale=1, interp="cubic", A=0):
    ...
    features = np.array(
        [vec_to_grid(y, N=random_dim) for y in features])  # Convert to (samples, random_dim, random_dim)

    t_grid = np.linspace(begin, end, Nt)  # Time dimension
    x_grid = np.linspace(begin, end, Nx)  # Spatial dimension

    x_data = space.eval_u(features, t_grid, x_grid)  # Calculate interpolation
...
class GRF(object):
    def __init__(self, begin=0, end=1, length_scale=5, N=1000, interp="cubic"):
        ...
        self.L = np.linalg.cholesky(
            self.K + 1e-12 * np.eye(self.N ** 2))  # Calculate Cholesky decomposition of covariance matrix
    ...
    def eval_u(self, ys, t, x):
        ...
        for i in range(ys.shape[0]):  # Iterate through each sample
...
def show(model, tx_grid,
    ...
    y_test = remove_2d_outer(torch.from_numpy(FD_RD_nonlinear(f_test, Nd=1, Nt=Config.Nt - 1, Nx=Config.Nx - 1)))
...
if __name__ == "__main__":
    ...
    u_real = FD_RD_nonlinear(f_grf, Nd=Config.n_samples, Nt=Config.Nt - 1, Nx=Config.Nx - 1)
    ...
    for i in range(Config.n_test_samples):
        ...
        show(model, tx_grid,         colormap='RdYlBu_r',  # 2D热力图
         colormap_3d='inferno',  # 3D曲面图
         colormap_error='RdBu_r')  # 误差图)
